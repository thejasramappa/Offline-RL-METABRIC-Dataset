...
model_path = os.path.join(SCRIPT_DIR, "..", "model", "chemo_cql_brca_metabric_model.pt")
cql.load_model(model_path)

action = int(cql.predict(state)[0])
msg = "Recommend Chemotherapy" if action == 1 else "Avoid Chemotherapy"

# Print only the message without action number
print(f"Prediction: {msg}")

# Optional logging
log_path = os.path.join(SCRIPT_DIR, "..", "data", "live_predictions_chemo_log.csv")
