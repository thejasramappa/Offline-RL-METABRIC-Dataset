...
model_path = os.path.join(SCRIPT_DIR, "..", "model", "hormone_cql_brca_metabric_model.pt")
cql.load_model(model_path)

action = int(cql.predict(state)[0])
msg = "Recommend Hormone Therapy" if action == 1 else "Avoid Hormone Therapy"

# Print only the message without action number
print(f"Prediction: {msg}")

# Optional logging (now includes ID and Patient Name)
log_path = os.path.join(SCRIPT_DIR, "..", "data", "live_predictions_hormone_log.csv")
...
# Append without header if file does not exist
